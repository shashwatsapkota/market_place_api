...
describe Product do
  ...
  describe '.above_or_equal_to_price' do
    ...
    it 'returns the products which are above or equal to the price' do
      expect(Product.equal_or_above_to_price(100).sort).to match_array([@product1, @product3])
    end
  end
  ...
  describe '.below_or_equal_to_price' do
    ...
    it 'returns the products which are below or equal to the price' do
      expect(Product.below_or_equal_to_price(100).sort).to match_array([@product2, @product4, @product1])
    end
  end
  ...
  describe '.search' do
    ...
    context 'when an empty hash is present' do
    ...
    context 'when product_ids is present' do
      it 'returns the product from the ids' do
        search_hash = { product_ids: [@product1.id, @product3.id] }
        expect(Product.search(search_hash)).to match_array([@product1, @product3])
      end
    end
  end
end
